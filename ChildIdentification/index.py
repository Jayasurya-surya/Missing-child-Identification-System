...
def authority_search2():
	try:
		...
		if len(namelist) == 0 or namelist[0] == "unknown":
			...
			return render_template("results.html", list=list, img_data=encoded_img_data.decode('utf-8'),
...
def user_search2():
	try:
		...
		if len(namelist) == 0 or namelist[0] == "unknown":
			...
			cursor.execute("""
			    SELECT u.email, p.photo 
			    FROM uploadphotos p 
			    JOIN users u ON p.unm = u.uid 
			    WHERE p.cid = %s AND p.status = 'Resolved'
			""", (namelist[0],))
			...
			msg = MIMEMultipart()
			msg['From'] = 'suryagowda000143@gmail.com'
			msg['To'] = 'hemanthadevadiga111@gmail.com'
			msg['Subject'] = 'Your image has been resolved'

			body = 'Your image has been resolved. Here is a copy of the image:'
			msg.attach(MIMEText(body))

			with open(photo_path, 'rb') as f:
				img = MIMEImage(f.read())
				img.add_header('Content-Disposition', 'attachment', filename="resolved_image.jpg")
				msg.attach(img)

			smtp_server = 'smtp.gmail.com'
			smtp_port = 587
			smtp_username = 'suryagowda000143@gmail.com'
			smtp_password = 'fwnofgjvnjgutndp'
			...
			return render_template("viewresults.html", list=list, img_data=img_data.decode('utf-8'),
			                       img_data2=img_data2.decode('utf-8'))
